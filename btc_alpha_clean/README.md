...
## ⚠️ v20: Critical Data Leakage Fix

This version includes **critical fixes** to prevent data leakage:

1. **Feature Time-Shift**: All features shifted by 1 bar so we only use data available BEFORE the prediction bar
2. **Liquidation Blacklist**: Removed liquidation features (they're reactive, not predictive)  
3. **Increased Embargo**: 12-bar (48h) embargo between train/val/test splits
...
Results that seem "worse" are actually honest and tradeable!
