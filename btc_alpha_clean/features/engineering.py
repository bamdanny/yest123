...
class FeatureGenerator:
    ...
    def generate_all_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ═══════════════════════════════════════════════════════════════════════════
        # CRITICAL: PREVENT LOOKAHEAD BIAS / DATA LEAKAGE
        # ═══════════════════════════════════════════════════════════════════════════
        # All features must be shifted by 1 bar so we only use information
        # available BEFORE the bar we're predicting. Using bar T data to predict
        # bar T direction is cheating.
        #
        # Example: If we're at bar T and want to predict if price goes up,
        # we can only use data from bar T-1 and earlier.
        # ═══════════════════════════════════════════════════════════════════════════
        
        logger.info("Applying lookahead prevention: shifting all features by 1 bar...")
        
        # Identify feature columns (exclude raw OHLCV and metadata)
        non_feature_cols = {'timestamp', 'open', 'high', 'low', 'close', 'volume', 
                           'quote_volume', 'trades', 'taker_buy_volume', 
                           'taker_buy_quote_volume', 'taker_sell_volume', 'taker_buy_ratio'}
        feature_cols = [col for col in new_cols if col not in non_feature_cols]
        
        # Shift all feature columns by 1 bar
        for col in feature_cols:
            df[col] = df[col].shift(1)
        
        # Drop first row (now NaN due to shift)
        df = df.iloc[1:].copy()
        
        logger.info(f"Lookahead prevention complete. Final shape: {df.shape}")
    ...
    def generate_lagged_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ═══════════════════════════════════════════════════════════════════════════
        # LAGGED LIQUIDATION FEATURES - CRITICAL FOR ALPHA
        # These are NOT leakage - they use PAST liquidation data to predict FUTURE moves
        # ═══════════════════════════════════════════════════════════════════════════
        df = self._generate_lagged_liquidation_features(df)
        
        logger.info(f"    Generated lagged features for {len(key_features[:30])} base features")
        return df
    
    def _generate_lagged_liquidation_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Generate lagged liquidation features - these have REAL predictive value.
        
        WHY these are valuable (not leakage):
        - Liquidity hunting: Market makers target stop-loss clusters
        - Cascade zones: Past liquidation levels show where future cascades may trigger
        - Positioning insight: Long/short liq imbalance from past bars shows positioning
        
        IMPORTANT: All features are properly LAGGED (shifted) to prevent lookahead bias.
        """
        # Find raw liquidation columns (same-bar data we'll transform into lagged features)
        liq_cols = [c for c in df.columns if 'liq' in c.lower() and 
                    not any(x in c for x in ['_lag_', '_sum_', '_mean_', '_zscore_', '_mom'])]
        
        if not liq_cols:
            logger.warning("    No liquidation columns found for lagged feature generation")
            return df
        ...
        for col in liq_cols:
            ...
            if 'ratio' in col.lower():
                base = 'liq_ratio'
            elif 'long' in col.lower() and 'liq' in col.lower():
                base = 'long_liq'
            elif 'short' in col.lower() and 'liq' in col.lower():
                base = 'short_liq'
            elif 'total' in col.lower() and 'liq' in col.lower():
                base = 'total_liq'
            else:
                base = 'liq_' + base
            
            # LAGGED VALUES (shift by N bars - NO leakage)
            for lag_bars in [1, 3, 6, 12, 24]:  # 4h, 12h, 24h, 48h, 96h
                lag_hours = lag_bars * 4  # Convert 4h bars to hours
                new_col = f'liq_feat_{base}_lag_{lag_hours}h'
                df[new_col] = df[col].shift(lag_bars)
                lagged_count += 1
            
            # ROLLING SUMS of PAST data (NO leakage - window looks backward)
            for window in [6, 12, 24, 42]:  # 24h, 48h, 96h, 168h
                window_hours = window * 4
                
                # Sum of past liquidations
                new_col = f'liq_feat_{base}_sum_{window_hours}h'
                df[new_col] = df[col].shift(1).rolling(window, min_periods=1).sum()
                lagged_count += 1
                
                # Mean of past liquidations
                new_col = f'liq_feat_{base}_mean_{window_hours}h'
                df[new_col] = df[col].shift(1).rolling(window, min_periods=1).mean()
                lagged_count += 1
                
                # Max of past liquidations (extreme events)
                new_col = f'liq_feat_{base}_max_{window_hours}h'
                df[new_col] = df[col].shift(1).rolling(window, min_periods=1).max()
                lagged_count += 1
            
            # Z-SCORES (how extreme is PAST data vs history - NO leakage)
            for window in [24, 42, 84]:  # 96h, 168h, 336h lookback
                window_hours = window * 4
                # Use shift(1) to ensure we only look at PAST data
                past_data = df[col].shift(1)
                mean = past_data.rolling(window, min_periods=10).mean()
                std = past_data.rolling(window, min_periods=10).std()
                
                new_col = f'liq_feat_{base}_zscore_{window_hours}h'
                df[new_col] = (past_data - mean) / (std + 1e-10)
                lagged_count += 1
            
            # MOMENTUM of liquidations (rate of change in past data)
            for mom_period in [1, 3, 6]:  # 4h, 12h, 24h momentum
                mom_hours = mom_period * 4
                past_data = df[col].shift(1)
                
                new_col = f'liq_feat_{base}_mom_{mom_hours}h'
                df[new_col] = past_data.diff(mom_period)
                lagged_count += 1
        
        logger.info(f"    Generated {lagged_count} lagged liquidation features from {len(liq_cols)} base columns")
        return df
