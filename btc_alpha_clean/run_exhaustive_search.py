...
class ExhaustiveSearchEngine:
    ...
    def __init__(
        ...
        # CRITICAL: Only exclude SAME-BAR liquidation features (data leakage)
        # LAGGED liquidation features (*_lag_24h, *_sum_168h) are KEPT - they have real alpha!
        SAME_BAR_LEAKY_FEATURES = [
        ...
        self.indicator_columns = [
            ...
            and col not in SAME_BAR_LEAKY_FEATURES
        ]
        
        # Count excluded features for transparency
        excluded_count = len([col for col in features.columns if col in SAME_BAR_LEAKY_FEATURES])
        if excluded_count > 0:
            logger.warning(f"  Excluded {excluded_count} same-bar liquidation features (data leakage)")
            logger.info(f"  Note: LAGGED liquidation features (*_lag_*, *_sum_*) are KEPT for their predictive value")
