...
class FeatureEngineer:
    ...
    def _add_oi_liq_interactions(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        IMPORTANT: We use LAGGED liquidation features (not same-bar) to avoid leakage.
        Historical liquidation patterns have legitimate predictive value:
        ...
        - Positioning insight (long/short imbalance from past bars)
        """
        
        # Find OI columns (these are safe)
        ...
        # Find LAGGED liquidation columns only (not same-bar!)
        # Look for features with lag, sum, or max suffix indicating historical data
        liq_cols = [c for c in df.columns if 
                    ('liq_ratio' in c.lower() or 'liq_usd' in c.lower())
                    and any(suffix in c.lower() for suffix in ['_lag_', '_sum_', '_max_', '_zscore'])]
        
        # Fallback: if no lagged liq features found, don't create interactions
        if not liq_cols:
            logger.info("  No lagged liquidation features found for OI×Liq interactions")
            return df
        ...
        if interactions_created > 0:
            logger.info(f"  Added {interactions_created} OI×Liquidation interactions (using lagged liq features)")
    ...
    def _add_momentum_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # NOTE: Using lagged liquidation features for legitimate alpha,
        # NOT same-bar liq_ratio (which is reactive/leaky)
        key_features = [
            'deriv_feat_cg_oi_aggregated_oi_close_change_1h',
            'deriv_feat_cg_oi_aggregated_oi_close_accel',
            'price_bb_width_50',
            # Liquidation features - use lagged versions if available
            'deriv_feat_cg_liquidation_aggregated_liq_ratio_sum_24h',
            'deriv_feat_cg_liquidation_history_liq_ratio_max_24h',
        ]
    ...
    def _add_cross_zscores(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        NOTE: We exclude same-bar liquidation from z-score calculations.
        Only OI features are used, as they're predictive (positioning) 
        rather than reactive (liquidation).
        """
        
        # Use OI features only - they show POSITIONING intent
        # NOT same-bar liquidation (which is REACTIVE)
        tier1_features = [
            'deriv_feat_cg_oi_aggregated_oi_close_change_1h',
            'deriv_feat_cg_oi_aggregated_oi_close_accel',
