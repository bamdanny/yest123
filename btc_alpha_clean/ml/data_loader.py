...
class DataLoader:
    ...
    def load_from_cache(self) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        # Get returns - try multiple approaches
        returns = None
        
        # Approach 1: Use pre-computed returns if available
        if 'returns' in cache:
            returns = cache['returns']
            logger.info(f"Using cached returns")
        ...
        # Approach 2: Compute from price data in cache  
            ...
            if 'close' in price.columns:
                ...
                logger.info(f"Generated returns from cached price")
        ...
        # Approach 3: Load price from parquet files
            ...
            for price_path in price_paths:
                if price_path.exists():
                    ...
                    if 'close' in price.columns:
                        returns = price['close'].pct_change()
                        logger.info(f"Generated returns from {price_path}")
                        break
        
        if returns is None:
            raise ValueError(
                "Could not generate returns. Cache needs either:\n"
                "  - 'returns' key\n"
                "  - 'price' key with close column\n"
                "  - Or price parquet in data_cache/binance/"
            )
        ...
        valid_idx = features.index.intersection(returns.dropna().index)
...
def load_data(config_path: str = None) -> Tuple[pd.DataFrame, pd.Series]:
    """
    Convenience function to load features and target.
