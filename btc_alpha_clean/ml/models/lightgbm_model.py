...
class LightGBMModel(BaseModel):
    ...
    def __init__(self, params: Dict[str, Any] = None):
        ...
        # Conservative defaults - prevent overfitting on small datasets
        default_params = {
            'objective': 'binary',
            'metric': 'auc',
            'num_leaves': 15,            # Shallow = less overfitting
            'learning_rate': 0.03,       # Slower learning for better generalization
            'n_estimators': 1000,        # More trees (early stopping will limit)
            'min_child_samples': 30,     # Higher = more regularization
            'min_data_in_leaf': 30,      # Same as min_child_samples
    ...
    def fit(
        ...
        if X_val is not None and y_val is not None:
            self.model.fit(
                ...
                callbacks=[lgb.early_stopping(100, verbose=False)]  # Wait 100 rounds
            )
